...
(function(){
  ...
  function appData(){try{return typeof DATA!=='undefined'?DATA:null}catch(e){return null}}
  function appMap(){try{return typeof map!=='undefined'?map:null}catch(e){return null}}
  function codeFor(p){return String(val(p,'id','name','title','code')||'').trim()}
  function recordsFor(p){
    const code=codeFor(p).toLowerCase(); const d=appData();
    const arr=d&&Array.isArray(d.records)?d.records:[];
  ...
  function historyHtml(p){
    ...
    return '<div class="history"><b>Monitoring history ('+rows.length+')</b>'+rows.slice(0,50).map(r=>{
      const date=val(r,'record_date','recordDate','date'); const species=val(r,'species_caught','speciesCaught','species','catch')||'No catch';
      const status=val(r,'trap_status','trapStatus','status'); const condition=val(r,'trap_condition','trapCondition','condition');
      const bait=val(r,'bait_at_arrival','baitArrival','bait'); const strikes=val(r,'strikes'); const by=val(r,'recorded_by','recordedBy','operator','user_name'); const notes=val(r,'record_notes','recordNotes','notes');
  ...
  function bind(){
    const m=appMap(); const d=appData(); if(!m||!d||window.__kpHistoryBound)return false;
    window.__kpHistoryBound=true;
    m.on('click','traps',function(e){
      const p=e.features&&e.features[0]&&e.features[0].properties; if(!p)return;
      const trap=(d.traps||[]).find(t=>String(val(t,'id','title','name')||'').trim()===String(p.id||p.name||'').trim()); if(!trap)return;
      const popup=document.createElement('div'); popup.className='popup';
      popup.innerHTML='<h3>'+esc(codeFor(trap))+'</h3><div class="details"><div class="detail"><b>Type:</b><span>'+esc(val(trap,'type'))+'</span></div><div class="detail"><b>Line:</b><span>'+esc(val(trap,'line','line_name','lineName')||'—')+'</span></div></div>'+historyHtml(trap);
      new maplibregl.Popup({maxWidth:'360px'}).setLngLat(e.lngLat).setDOMContent(popup).addTo(m);
    }); return true;
  }
